    ...
    : IRustPlusSocket, IDisposable
{
    ...
    private readonly ConcurrentDictionary<uint, TaskCompletionSource<AppMessage>> _pendingResponseMessages = new();
    ...
    /// <summary>
    /// Asynchronously sends a request and waits for the matching response message by sequence id.
    /// </summary>
    /// <param name="request">The <see cref="AppRequest"/> to send.</param>
    /// <returns>A task containing the matching <see cref="AppMessage"/> response.</returns>
    public async Task<AppMessage> SendRequestForResponseAsync(AppRequest request)
    {
        var tcs = new TaskCompletionSource<AppMessage>();
        var seq = Interlocked.Increment(ref _seq);
        ...
        _pendingResponseMessages[seq] = tcs;
        _sendQueue.Enqueue(request);
        ...
        while (IsConnected() && !CancellationToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (message.Response?.HasSeq == true
                        && _pendingResponseMessages.TryRemove(message.Response.Seq, out var pendingResponseTcs))
                    {
                        pendingResponseTcs.SetResult(message);
                        continue;
                    }
                }
