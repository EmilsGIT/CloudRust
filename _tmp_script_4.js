    ...
    function getSavedMySystemPlacedDevices() {
      try {
        const raw = localStorage.getItem(mySystemPlacedDevicesStorageKey);
        if (!raw) {
          return [];
        }
    ...
    function saveMySystemPlacedDevices(items) {
      localStorage.setItem(mySystemPlacedDevicesStorageKey, JSON.stringify(items));
    }
    ...
    function getSavedSwitches() {
      try {
        const raw = localStorage.getItem(switchStorageKey);
        if (!raw) {
          return [];
        }
    ...
    function saveSwitches(items) {
      localStorage.setItem(switchStorageKey, JSON.stringify(items));
    }
    ...
    function saveMonitors(items) {
      localStorage.setItem(monitorStorageKey, JSON.stringify(items));
    }

    function getSavedVirtualMonitors() {
      try {
        const raw = localStorage.getItem(virtualMonitorStorageKey);
        if (!raw) {
          return [];
        }
    ...
    function saveVirtualMonitors(items) {
      localStorage.setItem(virtualMonitorStorageKey, JSON.stringify(items));
    }

    function getVirtualMonitorLinks() {
      try {
        const raw = localStorage.getItem(virtualMonitorLinksStorageKey);
        if (!raw) {
          return {};
        }
    ...
    function saveVirtualMonitorLinks(links) {
      localStorage.setItem(virtualMonitorLinksStorageKey, JSON.stringify(links));
    }
    ...
    function getSavedVirtualSwitches() {
      try {
        const raw = localStorage.getItem(virtualSwitchStorageKey);
        if (!raw) {
          return [];
        }
    ...
    function saveVirtualSwitches(items) {
      localStorage.setItem(virtualSwitchStorageKey, JSON.stringify(items));
    }

    function getVirtualSwitchLinks() {
      try {
        const raw = localStorage.getItem(virtualSwitchLinksStorageKey);
        if (!raw) {
          return {};
        }
    ...
    function saveVirtualSwitchLinks(links) {
      localStorage.setItem(virtualSwitchLinksStorageKey, JSON.stringify(links));
    }

    function getVirtualSwitchLinkJoints() {
      try {
        const raw = localStorage.getItem(virtualSwitchLinkJointsStorageKey);
        if (!raw) {
          return {};
        }
    ...
    function saveVirtualSwitchLinkJoints(joints) {
      localStorage.setItem(virtualSwitchLinkJointsStorageKey, JSON.stringify(joints));
    }
    ...
    function renderMonitorLinkLines() {
      ...
      function getVirtualMonitorLinkJoints() {
        try {
          const raw = localStorage.getItem(virtualMonitorLinkJointsStorageKey);
          if (!raw) {
            return {};
          }
          const parsed = JSON.parse(raw);
      ...
      function saveVirtualMonitorLinkJoints(joints) {
        localStorage.setItem(virtualMonitorLinkJointsStorageKey, JSON.stringify(joints));
      }
