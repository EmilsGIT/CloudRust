    ...
    const pairingListenerBadge = document.getElementById('pairingListenerBadge');
    const pairingSummary = document.getElementById('pairingSummary');
    const pairingServerCard = document.getElementById('pairingServerCard');
    const pairingRecentEvents = document.getElementById('pairingRecentEvents');
    const pairingHelperStatus = document.getElementById('pairingHelperStatus');
    const pairingHelperOutput = document.getElementById('pairingHelperOutput');
    ...
    function ensurePairingStatusPoll() {
      ...
      pairingStatusPollTimer = setInterval(() => {
        ...
        if (!helperRunning && !pairingTabActive) {
          clearPairingStatusPoll();
          return;
        }
    ...
    function renderPairingStatus() {
      if (!pairingStatusText || !pairingListenerBadge || !pairingSummary || !pairingServerCard || !pairingRecentEvents || !pairingHelperStatus || !pairingHelperOutput) {
        return;
      }

      if (!authenticatedUser || authenticatedUser.isAdmin) {
        pairingListenerBadge.dataset.state = 'idle';
        pairingListenerBadge.textContent = 'Idle';
        pairingStatusText.textContent = 'Sign in with a regular account to manage your Rust+ listener.';
        pairingSummary.innerHTML = '';
        pairingServerCard.innerHTML = '';
        pairingRecentEvents.innerHTML = '<li class="live-observer-log-empty">No session pairing activity yet.</li>';
        pairingHelperStatus.textContent = 'Run the helper to launch Chrome, sign in to Steam, and import a fresh Rust+ config for this account.';
        pairingHelperOutput.innerHTML = '<li class="live-observer-log-empty">No helper activity yet.</li>';
        if (pairingConfigInput) {
          pairingConfigInput.value = '';
          pairingConfigInput.disabled = true;
        }
        if (pairingSteamLoginButton) {
      ...
      const badgeState = listenerConnected ? 'connected' : hasConfig ? 'ready' : 'idle';

      pairingListenerBadge.dataset.state = badgeState;
      pairingListenerBadge.textContent = listenerConnected ? 'Connected' : hasConfig ? 'Configured' : 'Idle';
      pairingSummary.innerHTML = [
        `<span class="admin-summary-pill">Config: ${hasConfig ? 'saved' : 'missing'}</span>`,
        `<span class="admin-summary-pill">Listener: ${listenerConnected ? 'connected' : 'stopped'}</span>`,
        `<span class="admin-summary-pill">Server: ${hasServerPairing ? 'paired' : 'waiting'}</span>`
      ].join('');

      if (snapshot.serverPairing) {
        pairingServerCard.innerHTML = `
          <h4 class="pairing-subtitle">Current Server Pairing</h4>
          <div class="admin-storage-block">
            <pre>${escapeHtml(JSON.stringify(snapshot.serverPairing, null, 2))}</pre>
          </div>`;
      } else {
        pairingServerCard.innerHTML = '<p class="info-footnote">No server pairing captured for this account yet. Start the listener, then pair the server in Rust+.</p>';
      }

      const events = Array.isArray(snapshot.recentEvents) ? snapshot.recentEvents.slice().reverse().slice(0, 12) : [];
      pairingRecentEvents.innerHTML = events.length
        ? events.map((event) => {
            const occurredAt = event?.occurredAtUtc ? formatUiTime(event.occurredAtUtc) : 'Unknown time';
            return `<li class="live-observer-log-entry">${escapeHtml(`${occurredAt}  ${event?.type || 'unknown'}`)}</li>`;
          }).join('')
        : '<li class="live-observer-log-empty">No session pairing activity yet.</li>';

      pairingHelperStatus.textContent = steamLogin.lastMessage
        || (steamLoginRunning
          ? 'Steam login helper is running. Complete the Chrome sign-in window.'
          : 'Run the helper to launch Chrome, sign in to Steam, and import a fresh Rust+ config for this account.');

      const helperOutput = Array.isArray(steamLogin.recentOutput) ? steamLogin.recentOutput : [];
      pairingHelperOutput.innerHTML = helperOutput.length
        ? helperOutput.slice().reverse().map((entry) => {
            const entryOccurredAt = entry?.occurredAtUtc || entry?.OccurredAtUtc || null;
            const entryMessage = entry?.message || entry?.Message || '';
            const occurredAt = entryOccurredAt ? formatUiTime(entryOccurredAt) : 'Unknown time';
            return `<li class="live-observer-log-entry">${escapeHtml(`${occurredAt}  ${entryMessage}`)}</li>`;
          }).join('')
        : '<li class="live-observer-log-empty">No helper activity yet.</li>';

      if (pairingConfigInput) {
        pairingConfigInput.disabled = false;
      }
      if (pairingSteamLoginButton) {
        pairingSteamLoginButton.disabled = steamLoginRunning;
      }
      if (pairingSaveButton) {
        pairingSaveButton.disabled = steamLoginRunning;
      }
      if (pairingSaveStartButton) {
        pairingSaveStartButton.disabled = steamLoginRunning;
      }
      if (pairingStopButton) {
        pairingStopButton.disabled = steamLoginRunning || (!hasConfig && !listenerConnected);
      }
    ...
    async function loadPairingConfig() {
      ...
      pairingConfigInput.value = typeof payload?.configJson === 'string' ? payload.configJson : '';
    }
    ...
    async function loadPairingStatus(statusMessage) {
      ...
      try {
        ...
        if (pairingStatusSnapshot?.steamLogin?.isRunning) {
          ensurePairingStatusPoll();
        } else {
          clearPairingStatusPoll();
        }
    ...
    async function runPairingSteamLoginFlow() {
      ...
      try {
        await fetchJson('/api/pairing/steam-login/start', {
          method: 'POST'
        });
        ensurePairingStatusPoll();
    ...
    function startAuthenticatedApp() {
      ...
      infoEventsIntervalId = setInterval(loadInfoEvents, 15000);
      teamStatusIntervalId = setInterval(loadTeamStatus, 30000);
    }
    ...
    function renderVirtualMonitorCombinedItems(virtualMonitorId) {
      ...
      renderItemSlots(virtualGrid, combinedItems, 48);
    ...
    function resetSwitchPairingUi() {
      ...
      if (fcmStatus) {
        fcmStatus.textContent = 'Name your smart switch, then pair it in Rust+ to enable Add.';
      }
    ...
    function resetMonitorPairingUi() {
      ...
      if (monitorFcmStatus) {
        monitorFcmStatus.textContent = 'Name your storage monitor, then pair it in Rust+ to enable Add.';
      }
    ...
    function connectFcmEventStream() {
      ...
      source.onopen = () => {
        clearFcmReconnectTimer();
        if (fcmStatus) {
          fcmStatus.textContent = 'Connected. Name your smart switch, then pair it in Rust+ to enable Add.';
        }
        if (monitorFcmStatus) {
          monitorFcmStatus.textContent = 'Connected. Name your storage monitor, then pair it in Rust+ to enable Add.';
        }
      };
